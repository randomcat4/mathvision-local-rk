...
| 类别 | 含义 | 可否计入数学分母 |
|---|---|---|
| 原始证据 | 审计文件、验证文件或 GitHub 提交直接支持的节点 | 依原审计裁决 |
| 账本重建 | 从 132 个工作单元状态和轮次账本重建的过程节点 | 只能说明过程存在 |
| 压力副本 | 为模拟高消息密度复制的展示事件 | 不可 |

每个回放线程分别保存 `mathematicalScope`、`proofState` 和 `verificationState`。一次运行成功、局部命题已证和 Gao 母题已解决因此不能互相替代。

时间也不压成一个字段：本地工件时间、任务记录时间和 GitHub 提交时间各自保留来源。GitHub 时间只证明证据何时进入仓库。
