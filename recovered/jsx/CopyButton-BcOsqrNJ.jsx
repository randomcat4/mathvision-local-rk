...
const B = ({
  ...
  const d = o ? <E /> : l ? j.createElement(l, {
    fontSize: "small"
  }) : <R />;
  if (C) {
    return <T tooltip={w} type="button" aria-label={s} disabled={t} disableRipple={true} onClick={() => {
      u();
    }} sx={[{
      boxShadow: n => n.shadows[8]
    }, ...(Array.isArray(i) ? i : [i])]} color="inherit">{d}</T>;
  } else {
    return <_ type="button" aria-label={s} title={s} disabled={t} disableRipple={true} onClick={() => {
      u();
    }} color="secondary" size={h} startIcon={d} sx={i} variant={m}><span>{o ? "Copied" : e}</span></_>;
  }
};
