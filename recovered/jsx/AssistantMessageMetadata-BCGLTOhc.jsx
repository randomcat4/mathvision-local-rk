...
const y = ({
  ...
  var p;
  var x;
  const e = l.modelId ?? ((p = l.usage) == null ? undefined : p.modelId) ?? null;
  ...
  if (!i && !c && !d && !s && !r) {
    ...
    return <_Component3 sx={w}>{i && <_Component2 label={`Answer ${i}`} color="primary" size="xs" variant="filled" />}{s && <_Component2 label="Agent" color="primary" size="xs" variant="filled" />}{t && <_Component2 label={t} variant="filled" color="primary" icon={g} />}{d && <_Component2 label={m} variant="filled" color="secondary" />}{r && <_Component2 label="Retries detected" variant="filled" color="warning" size="small" />}{l.proChatCall && <_Component2 label="Math Vision Pro" icon={<M />} color="warning" variant="filled" size="small" sx={{
        ml: "auto"
      }} />}</_Component3>;
  }
};
