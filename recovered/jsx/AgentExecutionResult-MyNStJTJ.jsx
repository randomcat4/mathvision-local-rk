...
const et = ({
  ...
  const {
    ...
    return {
      answerCitations: n,
      answerSources: u,
      missingAnswerCitationIds: ((t == null ? undefined : t.citationIds) ?? []).filter(a => !l.has(a)),
      missingAnswerSourceIds: [...o].filter(a => !p.has(a))
    };
  }, [t == null ? undefined : t.citationIds, r.citations, r.sources]);
  const d = m.useMemo(() => new Map(((t == null ? undefined : t.outputs) ?? []).map(i => [i.id, i])), [t == null ? undefined : t.outputs]);
  const [e, G] = m.useState(null);
  const w = m.useCallback(i => {
    if (!d.has(i)) {
  ...
  const y = m.useMemo(() => {
    ...
    const n = t.usedOperationOutputIds.filter(o => !i.has(o)).map(o => d.get(o)).filter(o => o !== undefined);
    if (n.length === 0) {
      return t.answer;
    }
...
See intermediate ${l.length === 1 ? "result" : "results"}: ${l.join(", ")}.`;
  ...
  const C = m.useMemo(() => F(y, r.markdownMathFormat ?? undefined), [y, r.markdownMathFormat]);
  const P = m.useMemo(() => ((t == null ? undefined : t.unresolvedItems) ?? []).map(i => {
  ...
  const B = m.useMemo(() => ({
    a: ({
      ...
      var N;
      const T = (N = l == null ? undefined : l.match(ot)) == null ? undefined : N[1];
      if (!T || !d.has(T)) {
        if (o === Z && l) {
          return <V className={a.className} href={l} id={a.id} isPreviewEnabled={A} label={u} />;
        } else {
          return <a href={l} title={o} onClick={p} {...a}>{u}</a>;
        }
      } else {
        return <X href={l} title={o} onClick={x => {
          if (p != null) {
            p(x);
          }
  ...
  if (t) {
    return <E sx={z}><H tasks={t.tasks} chatId={g} message={r} messageIndex={O} operations={t.operations} operationOutputs={t.outputs} operationOutputRequestKey={e == null ? undefined : e.requestKey} onOperationOutputSelect={w} requestedOperationOutputId={e == null ? undefined : e.outputId} areCitationPreviewsEnabled={A} areVariableHighlightsEnabled={j} areWriteupActionsEnabled={M} areWriteupExportsEnabled={I} onContentResizeInteraction={c} responseNumber={S} shouldPrepareWriteupPdf={L} writeupPdfPreparationPriority={_} /><R execution={t} areCitationPreviewsEnabled={A} areVariableHighlightsEnabled={j} areWriteupActionsEnabled={M} areWriteupExportsEnabled={I} chatId={g} message={r} messageIndex={O} onContentResizeInteraction={c} operationOutputRequestKey={e == null ? undefined : e.requestKey} requestedOperationOutputId={e == null ? undefined : e.outputId} responseNumber={S} shouldPrepareWriteupPdf={L} writeupPdfPreparationPriority={_} /><E component="section" aria-label="Final outcome" sx={st}><E direction="row" sx={rt}><D variant="subtitle2">Final outcome</D><_Component label={it[t.outcome]} color={nt[t.outcome]} size="xs" variant="filled" /></E><U areCitationPreviewsEnabled={A} components={B} content={C} citations={K} definitionHighlights={[]} mathLabelPrefix={S} mathSyntax={r.markdownMathFormat === "canonical-v1" ? "canonical" : "compatibility"} sources={$} />{k.length > 0 && <_Component2 severity="error">Referenced answer citations are unavailable: {k.join(", ")}.</_Component2>}{v.length > 0 && <_Component2 severity="error">Referenced answer sources are unavailable: {v.join(", ")}.</_Component2>}</E>{t.unresolvedItems.length > 0 && <_Component2 severity="warning"><_Component3>Unresolved items</_Component3><W sx={tt}>{P.map(({
            content: i,
            item: n
          }) => <J component="li" disablePadding={true} key={n}><U content={i} mathSyntax={r.markdownMathFormat === "canonical-v1" ? "canonical" : "compatibility"} mode="compact" /></J>)}</W></_Component2>}{t.safeError && <_Component2 severity="error"><_Component3>Agent error</_Component3>{h}</_Component2>}</E>;
  } else {
    return null;
  }
};
