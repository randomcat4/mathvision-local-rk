import { r as s, j as t, B as _Component, C as _, g as A } from "./index-BM3ZINIl.js";
...
const J = s.memo(({
  ...
  return <_Component sx={B} style={{
    minHeight: w ? undefined : `${y}px`
  }}><_Component component="canvas" ref={l} aria-label={`${e}, page ${i}`} sx={j} />{u ? <_Component sx={K}><_ size={28} /></_Component> : null}{a ? <_Component sx={Y}><_Component2 severity="error" variant="outlined">{a}</_Component2></_Component> : null}</_Component>;
});
const ue = ({
  ...
  if (m && !r) {
    return <_Component sx={E}><_ size={28} /></_Component>;
  } else if (g) {
    return <_Component sx={E}><_Component2 severity="error" variant="outlined">{g}</_Component2></_Component>;
  } else if (r) {
    return <_Component ref={u} sx={S}><_Component sx={G} style={{
        height: `${p.getTotalSize()}px`
      }}>{p.getVirtualItems().map(n => <_Component ref={p.measureElement} data-index={n.index} sx={L} style={{
          transform: `translateY(${n.start}px)`
        }} key={n.key}><J ariaLabel={e} canvasSx={O} estimatedPageHeight={C} pageNumber={n.index + 1} pdfDocument={r} renderErrorMessage={h} scale={l} /></_Component>)}</_Component></_Component>;
  } else {
    return <_Component sx={E}><_ size={28} /></_Component>;
  }
};
