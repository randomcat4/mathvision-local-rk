import { j as e, r as p, B as _Component, ac as T, ad as _Component3, M as _Component2, ae as S, q as P, af as A, S as _Component5, ag as I, T as _Component4, g as L, a as k, ah as R } from "./index-BM3ZINIl.js";
...
function Z({
  ...
  const w = p.useCallback(s => {
    ...
    const c = (m = u.current) == null ? undefined : m.querySelector(`[data-message-index="${s}"]`);
    if (!c) {
  ...
  if (o.isPending) {
    return <_Component sx={h}><_Component sx={x}><T text="Opening shared chat" subtext="Fetching the chat snapshot." tone="shared" sx={b} /></_Component></_Component>;
  }
  if (o.isError || !r) {
    const s = o.error ?? new Error("Shared chat was not returned.");
    return <_Component sx={h}><_Component sx={x}><_Component3 title="Shared chat unavailable" subtitle={P(s)} icon={<U />} sx={b}><_Component2 to="/" variant="contained" startIcon={<S fontSize="small" />} sx={{
            mt: 1
          }}>Back to App</_Component2></_Component3></_Component></_Component>;
  }
  const E = r.title ?? A;
  const d = r.messages;
  const f = Q(d);
  const v = F(d);
  return <_Component sx={h}><_Component5 sx={x}><_Component5 component="header" direction="row" sx={K}><_Component5 direction="row" sx={O}><I><C /></I><_Component5 sx={{
            minWidth: 0
          }}><_Component5 component="span" direction="row" sx={z}><B color="primary" sx={{
                fontSize: "1rem"
              }} /><_Component4 component="span" variant="sharedPageEyebrow" color="primary">Shared chat</_Component4></_Component5><_Component sx={G}><_Component4 component="h1" variant="sharedPageTitle">{E}</_Component4></_Component>{g ? <_Component sx={V}><_Component4 component="p" variant="sharedPageMeta" color="textSecondary">Updated {g}</_Component4></_Component> : null}</_Component5></_Component5><_Component2 to="/" variant="outlined" startIcon={<S fontSize="small" />} sx={{
          flex: "0 0 auto"
        }}>Open app</_Component2></_Component5>{d.length > 0 ? <_Component5 ref={u} component="main" spacing={0.35} sx={Y}>{d.map((s, a) => {
          var c;
          var m;
          return <div data-message-index={a} key={J(s, a)}><N macros={v[a]}>{s.role === "assistant" ? <W message={s} messageIndex={a} responseNumber={(c = f.get(a)) == null ? undefined : c.responseNumber} availableModels={[]} areCitationPreviewsEnabled={false} areWriteupActionsEnabled={false} /> : <D message={s} queryNumber={(m = f.get(a)) == null ? undefined : m.queryNumber} isLatestUserMessage={false} areLatestUserActionsDisabled={true} onRetryLatestUserMessage={M} onEditLatestUserMessage={M} onQuoteClick={w} />}</N></div>;
        })}</_Component5> : <_Component3 title="No messages yet" subtitle="This shared chat does not contain any messages." icon={<C />} sx={{
        minHeight: "28rem",
        mt: 2
      }} />}</_Component5></_Component>;
}
function _Component6({
  chatId: t
}) {
  return <$ chatId={t}><Z chatId={t} /></$>;
}
function Je() {
  const {
    chatId: t
