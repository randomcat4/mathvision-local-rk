import { ab as j, am as I, r as u, F as M, j as i, H as B, I as z, K as S, T as F, aQ as A, G as D, a8 as x, a3 as C, aV as N, E as O, _ as R, A as U } from "./index-BM3ZINIl.js";
import { S as V } from "./FormControlLabel-CQvCddOM.js";
import { D as _Component4, a as E, b as H, c as W } from "./MChip-DiH8NXBU.js";
function _(t) {
...
const G = u.forwardRef(function (o, s) {
  ...
  const n = L(c);
  return <X component="p" variant="body1" color="textSecondary" ref={s} ownerState={c} className={B(n.root, r)} {...a} classes={n} />;
});
function K(t) {
  return j("MuiSwitch", t);
}
const e = I("MuiSwitch", ["root", "edgeStart", "edgeEnd", "switchBase", "colorPrimary", "colorSecondary", "sizeSmall", "sizeMedium", "checked", "disabled", "input", "thumb", "track"]);
const Q = t => {
  const {
    classes: o,
    edge: s,
...
const Y = S(V, {
  ...
  variants: [...Object.entries(t.palette).filter(N(["light"])).map(([o]) => ({
    props: {
      color: o
    },
    style: {
      [`&.${e.checked}`]: {
        ...
        "&:hover": {
          backgroundColor: t.alpha((t.vars || t).palette[o].main, (t.vars || t).palette.action.hoverOpacity),
          "@media (hover: none)": {
            backgroundColor: "transparent"
          }
        },
        [`&.${e.disabled}`]: {
          color: t.vars ? t.vars.palette.Switch[`${o}DisabledColor`] : `${t.palette.mode === "light" ? t.lighten(t.palette[o].main, 0.62) : t.darken(t.palette[o].main, 0.55)}`
        }
      },
      [`&.${e.checked} + .${e.track}`]: {
...
const nt = u.forwardRef(function (o, s) {
  ...
  return <$ {...k}><Y type="checkbox" icon={v} checkedIcon={v} ref={s} ownerState={p} {...m} classes={{
      ...h,
      root: h.switchBase
    }} slots={{
      ...(g.switchBase && {
...
function _Component3({
  ...
  return <_Component4 open={t} onClose={w ? undefined : r} aria-labelledby={v} aria-describedby={y} maxWidth="xs" fullWidth={true}><E sx={{
      textAlign: "center"
    }} id={v}>{o}</E><H sx={{
      textAlign: "center"
    }}><G id={y} variant="body2">{s}</G></H><W><R type="button" color="secondary" variant={g} disabled={w} onClick={r} fullWidth={true}>{a}</R><R type="button" color={m} variant="contained" disabled={w} startIcon={p} autoFocus={l} onClick={() => void T()} fullWidth={true}>{w ? f ?? d : d}</R></W></_Component4>;
}
