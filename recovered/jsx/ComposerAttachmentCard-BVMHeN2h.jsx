import { A as w, j as o, r as l, B as _Component2, T as _Component, V as R, a6 as S, g as a } from "./index-BM3ZINIl.js";
...
const O = w(<path d="M18.3 5.71a.996.996 0 0 0-1.41 0L12 10.59 7.11 5.7a.996.996 0 0 0-1.41 0c-.39.39-.39 1.02 0 1.41L10.59 12 5.7 16.89c-.39.39-.39 1.02 0 1.41s1.02.39 1.41 0L12 13.41l4.89 4.89c.39.39 1.02.39 1.41 0s.39-1.02 0-1.41L13.41 12l4.89-4.89c.38-.38.38-1.02 0-1.4" />);
const U = e => r => ({
...
const J = ({
  ...
  const x = h(e.storage === "stored" ? e.attachmentId : null, e.storage === "stored" && e.kind === "image");
  ...
  const L = n => {
    ...
    g(null);
    d(true);
    (e.storage === "local" ? e.file.text() : x.load().then(t => t.text())).then(t => {
      if (p === i.current) {
        g(t);
      }
    }).catch(t => {
  ...
  const c = <B variant="inset" sx={U(s)} role={s ? "button" : undefined} tabIndex={s ? 0 : undefined} aria-label={s ? `Preview ${e.name}` : undefined}><I kind={e.kind} name={e.name} href={v} /><_Component2 sx={W}><T name={e.name} /><_Component variant="caption" color="textSecondary">{e.kind === "image" && "Photo"}{e.kind === "file" && !u && "File"}{u && "Pasted text"}</_Component></_Component2><R sx={z} aria-label={`Remove ${e.name}`} disabled={r} disableRipple={true} onClick={n => {
      n.stopPropagation();
      m();
    }} onKeyDown={n => {
      n.stopPropagation();
    }}><O fontSize="small" /></R></B>;
  if (s) {
    return <l.Suspense fallback={c}><$ name={e.name} source={C} isLoading={y} errorMessage={P} onOpenChange={L} triggerTooltip={e.name}>{c}</$></l.Suspense>;
  } else {
    return <S title={e.name} describeChild={true}>{c}</S>;
  }
};
export { J as C };
