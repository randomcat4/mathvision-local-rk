import { E as re, ax as ie, aj as ce, g as O, r as n, b1 as le, j as e, B as _Component4, _ as M, T as _Component, C as _Component7, S as _Component2, p as de, b2 as me, t as G, ai as ue, s as X, V as _Component8 } from "./index-BM3ZINIl.js";
...
const Ve = t => ie.post(ce, `latex/documents/${encodeURIComponent(t)}/chat`, {}).then(i => ge(i));
const Ue = {
  minWidth: 0,
  minHeight: 0,
  height: "100%",
...
const ot = ({
  ...
  const m = i.role === "user";
  const S = i.markdownMathFormat === "canonical-v1" ? "canonical" : "compatibility";
  if (!f && c.length === 0 && !l) {
    return null;
  } else {
    return <_Component4 sx={qe(m)}><_Component2 direction="row" spacing={0.5} sx={{
        alignItems: "center"
      }}><_Component variant="caption" color="textSecondary">{m ? "You" : "Assistant"}</_Component>{l && <Be size="xs" color={l.status === "pending" ? "warning" : l.status === "applied" ? "success" : "error"} label={`${l.status[0].toUpperCase()}${l.status.slice(1)} proposal`} />}</_Component2><Te type={m ? "user" : "assistant"} sx={Ge}>{c.length > 0 && <Re attachments={c} />}{f && <_Component3 content={f} mathSyntax={S} mode="compact" tone={m ? "user" : "assistant"} />}{!m && <Le sources={i.sources} />}</Te></_Component4>;
  }
};
...
const nt = ({
  ...
  const p = n.useMemo(() => {
    var T;
    const s = (T = [...r.messages].reverse().find(h => h.role === "assistant")) == null ? undefined : T.modelId;
  ...
  const {
    ...
    selectedExecutionMode: p,
    selectedReasoningEffort: (p == null ? undefined : p.reasoningEfforts[0]) ?? "none",
  ...
  const N = le(r);
  const V = !i.trim() && u.length === 0 || L || p === null || C || o || c || !!P || !!r.inputSafetyBlockedAt;
  n.useLayoutEffect(() => {
    const s = x.current;
    if (!!s && !!j.current) {
  ...
  const se = n.useCallback(s => {
    s.preventDefault();
    W();
  }, [W]);
  const Q = n.useCallback(s => {
  ...
  const ae = n.useMemo(() => u.map(s => <_Component5 attachment={s} disabled={c || o} onRemove={() => I(s.id)} key={s.id} />), [u, c, o, I]);
  return <e.Fragment><_Component4 ref={x} sx={Qe} onScroll={te}>{$ > 0 && <M type="button" size="small" variant="text" onClick={() => {
        y(s => s + Z);
      }}>Show {Math.min($, Z)} earlier messages</M>}{r.messages.length === 0 && !C && <_Component variant="body2" color="textSecondary" align="center">Ask about this project, or tell the chat to edit the contents of any text file. Proposed changes are shown for review before they are applied.</_Component>}{K.map((s, T) => <_Component6 documentId={r.latexDocumentId} message={s} key={`${s.role}:${s.timestamp}:${r.messages.length - K.length + T}`} />)}{N && <A severity="error">{N}</A>}{C && <_Component4 sx={tt}><_Component7 size={16} /><_Component variant="body2" color="textSecondary">Working with the project context…</_Component></_Component4>}</_Component4><_Component4 component="form" sx={Xe} onSubmit={se}><input ref={D} type="file" accept={Ie} multiple={true} disabled={C || o || c} hidden={true} onChange={B} />{!!a && <A severity="error">Chat modes could not be loaded.</A>}{C && <A severity="info">{_e}</A>}{U && <A severity="error">{U}</A>}{P && <A severity="error">{P}</A>}{u.length > 0 && <_Component4 sx={Je}>{ae}</_Component4>}<He sx={Ye} placeholder="Ask a question or describe the file edits you want" value={i} disabled={C || o || c} variant="standard" fullWidth={true} multiline={true} minRows={2} maxRows={6} slotProps={oe} onChange={s => {
        f(s.target.value);
      }} /><_Component4 sx={Ze}><_Component4 sx={et}><M type="button" size="small" variant="text" startIcon={<Oe size={16} />} disabled={C || o || c} onClick={z}>Attach</M><_Component variant="caption" color="textSecondary">Enter to send · Shift+Enter for a new line</_Component></_Component4>{!C && <M type="submit" size="small" variant="contained" disabled={V}>{c ? "Uploading" : "Send"}</M>}{C && <M type="button" size="small" variant="outlined" disabled={k} onClick={() => {
          F();
        }}>{k ? "Canceling" : "Cancel"}</M>}</_Component4></_Component4></e.Fragment>;
};
...
const it = ({
  ...
  }, [(F = a == null ? undefined : a.activeRun) == null ? undefined : F.id, (k = a == null ? undefined : a.latestRun) == null ? undefined : k.status, a == null ? undefined : a.messageRevision, t, g, y]);
  n.useEffect(() => {
    if (g) {
      l(D);
    }
  ...
  const {
    ...
    message: "This permanently removes all previous messages and dismisses pending proposed edits. Your LaTeX project files are not changed.",
  ...
  return <_Component4 sx={Ue}><_Component4 component="header" sx={$e}><_Component4 sx={Ke}><_Component variant="subtitle2" color="textPrimary">Project chat</_Component><_Component variant="caption" color="textSecondary">Revision {i}</_Component></_Component4><_Component4 sx={Ne}>{a && a.messages.length > 0 && <M type="button" size="small" variant="text" startIcon={<Y size={15} />} disabled={!!a.activeRun || I.isPending} onClick={() => {
          w("clear");
        }}>Clear</M>}{E && <_Component8 tooltip="Hide project chat panel" aria-label="Hide project chat panel" disabled={f} onClick={E}><Fe size={17} /></_Component8>}</_Component4></_Component4>{B && <_Component4 sx={J}><_Component7 size={24} /><_Component variant="body2" color="textSecondary">Opening project chat</_Component></_Component4>}{z && <_Component4 sx={J}><A severity="error">The project chat could not be opened.</A><M type="button" size="small" variant="outlined" onClick={() => {
        r.refetch();
        if (x) {
          j.refetch();
        }
      }}>Retry</M></_Component4>}{!B && !z && a && <Ae value={_}><_Component9 chat={a} key={a.id} /></Ae>}{H}</_Component4>;
};
const us = n.memo(it);
