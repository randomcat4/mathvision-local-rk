...
import {
  A as _Component2,
  b as _Component3,
} from "./AgentTaskProgress-bXMst_Kr.js";
...
const tt = (e, s, a, c) => {
  ...
  if (new Set(a.map((i) => i.id)).size !== a.length) {
    throw new Error(
      "Agent task reset snapshot contains duplicate operation ids.",
    );
  }
  if (new Set(c.map((i) => i.id)).size !== c.length) {
    throw new Error(
      "Agent task reset snapshot contains duplicate operation output ids.",
    );
  }
  ...
  for (const i of c) {
    ...
    if (
      !l ||
      l.status !== i.status ||
      l.operation !== i.operation ||
      l.taskId !== i.taskId
    ) {
      throw new Error(
        "Agent task reset snapshot output does not match its operation.",
      );
    }
  }
  return {
...
const Et = (e, s, a, c) => {
  ...
  const f = s.tasks.map((n) => {
    ...
    if (
      u &&
      u.sequence > a &&
      u.revision > n.revision &&
      x &&
      x.revision >= u.revision
    ) {
      return x;
    } else {
      return n;
...
const bt = (e, s, a = [], c = []) => {
  ...
  if ((e == null ? undefined : e.workflowMode) === "agent") {
    ...
    if (l.length === 1) {
      const u =
        (w = ft.find((x) => x.value === l[0].operation)) == null
          ? undefined
          : w.label;
      if (!u) {
        throw new Error(
          `Missing label for Agent operation "${l[0].operation}".`,
        );
      }
      return `${n} ${u}...`;
    }
...
const Pt = ({ chatId: e, activeRun: s, refetchRunState: a }) => {
  ...
  L.useEffect(() => {
    ...
    const Y = (t) => {
      ...
      const d = N.get(t);
      let o = d ?? 0;
      const p = [...((r == null ? undefined : r.entries()) ?? [])]
        .sort(([m], [I]) => m - I)
        .map(([m, I]) => {
          o = Math.max(o, m);
          return I;
    ...
    const R = (t) => {
      ...
      const r =
        t.type === "delta" &&
        t.operationId !== undefined &&
        t.operationId !== null;
      const d =
        t.type === "delta" && !r && t.taskId !== undefined && t.taskId !== null;
      if (t.sequence !== undefined) {
        if (
          X.has(t.sequence) ||
          (t.sequence < H &&
    ...
    s == null ? undefined : s.workflowMode,
    e,
    a,
  ]);
...
const _Component8 = ({
  ...
  return (
    <At
      ...
      message={F ? {} : w}
      responseNumber={f}
      operation={F ? undefined : g}
      availableModels={i}
      streamText={n}
      statusLabel={u}
      workflowMode={(s == null ? undefined : s.workflowMode) ?? "single"}
      chatId={e}
      messageIndex={h}
      tasks={x}
      taskStreams={A}
...
const ThreadPendingAssistantMessage = ({ markdownLatexMacros: e, ...s }) => (
  <St macros={e}>
    <_Component8 {...s} />
  </St>
);
export { ThreadPendingAssistantMessage };
