...
const $ = (e) =>
  e.dataBase64
    ? q(e.contentType || "application/octet-stream", e.dataBase64)
    : null;
const f = (e) => ({
  minWidth: 0,
  width: "100%",
...
const _Component5 = ({ attachment: e }) => {
  const a = P(e.attachmentId, !e.dataBase64);
  const o = a.objectUrl ?? $(e);
  if (o) {
    return (
      <_Component3 title={e.name} describeChild={true}>
        <_Component sx={k}>
          <_Component2
            href={o}
            target="_blank"
            ...
            sx={J}
          >
            <_Component component="img" src={o} alt={e.name} sx={K} />
          </_Component2>
          <_Component sx={L}>
            <_Component sx={A}>
              <C name={e.name} />
...
const _Component6 = ({ attachment: e }) => {
  const a = P(e.attachmentId, false);
  const o = a.objectUrl ?? $(e);
  const n = Y(e.name);
  const c = G(e.name);
  const v = l.useMemo(
    () => (!n || !e.dataBase64 ? null : z(e.dataBase64)),
    [e.dataBase64, n],
  );
  ...
  const u = (
    <r.Fragment>
      <H kind="file" name={e.name} href={o} />
      <_Component sx={Q}>
        <C name={e.name} />
        <F variant="caption" color={m ? "error" : "textSecondary"}>
  ...
  if (o) {
    ...
    return (
      <_Component3 title={e.name} describeChild={true}>
        <_Component4 variant="inset" sx={f}>
          {u}
        </_Component4>
      </_Component3>
    );
  ...
  return (
    <r.Fragment>
      {a.length > 0 && (
        <_Component sx={Z}>
          {a.map((n, c) => (
            <_Component5 attachment={n} key={n.id ?? `${n.name}-${c}`} />
          ))}
        </_Component>
      )}
      {o.length > 0 && (
        <_Component sx={X}>
          {o.map((n, c) => (
            <_Component6 attachment={n} key={n.id ?? `${n.name}-${c}`} />
          ))}
        </_Component>
      )}
    </r.Fragment>
...
const MessageAttachments = l.memo(ne);
export { MessageAttachments as M };
