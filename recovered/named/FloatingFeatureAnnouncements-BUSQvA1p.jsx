...
const je = () => {
  ...
  if (!e) {
    throw new Error(
      "Authenticated user token is missing its subject identifier.",
    );
  }
  ...
  const r = o.useMemo(
    () =>
      Array.from(
        new Set([...n.legacyAnnouncementIds, ...n.pendingAnnouncementIds]),
      ),
    [n],
  );
  ...
  const {
    ...
    onSuccess: () => {
      try {
        ...
        console.warn(
          "Could not remove reconciled announcement dismissal state.",
          u,
        );
      }
      s({
  ...
  const C = o.useCallback(
    (u) => {
      try {
        ...
        console.warn(
          "Could not remove a persisted announcement dismissal from the outbox.",
          N,
        );
      }
    },
...
function Me({ announcement: e, labelId: n, onClose: s }) {
  return (
    <_Component4 direction="row" spacing={1.5} sx={Re}>
      <_Component alt={e.authorName} src={be(e.authorAvatarSrc)} sx={Te} />
      <_Component4 spacing={1.25} sx={$e}>
        <_Component4
          direction="row"
          spacing={1}
          sx={{
        ...
        >
          <_Component4 sx={Pe}>
            <A id={n} variant="subtitle2" color="textPrimary" noWrap={true}>
              {e.authorName}
            </A>
            <A variant="caption" color="textSecondary">
              <_Component2 date={e.createdAt} /> ·{" "}
              <_Component3 value={e.createdAt} />
            </A>
          </_Component4>
          {s && (
            <_Component5
        ...
        <_Component6 content={De(e.content)} />
      </_Component4>
    </_Component4>
  );
}
...
function Oe({ announcement: e, onClose: n }) {
  ...
  return (
    ...
    >
      {e && <K announcement={e} labelId={s} onClose={n} />}
    </_Component7>
  );
}
o.memo(Oe);
...
const Qe = ({ openRequestId: e }) => {
  ...
  const { mutate: P } = Y({
    ...
    onSuccess: (l, m) => {
      C(m);
      n.setQueryData(O.announcements(false), (j) =>
        j == null ? undefined : j.filter((ee) => ee.id !== m),
      );
      se({
  ...
  if (U) {
    if (p === e) {
      ...
      return (
        <F sx={q}>
          <W role="alert" variant="floating" sx={G}>
            <_Component4 spacing={1.5} sx={Fe}>
              <_Component4 direction="row" sx={We}>
                <A variant="subtitle2" color="error">
                  Updates unavailable
                </A>
                <_Component5
                  tooltip="Close"
  ...
  return (
    <F sx={q}>
      <W id={Be} role="region" aria-labelledby={k} variant="floating" sx={G}>
        <K announcement={g} labelId={k} onClose={Z} />
        {c.length > 1 && (
          <t.Fragment>
            <_Component9 />
            <_Component4 direction="row" sx={ze}>
              <A variant="caption" color="textSecondary">
                {S + 1} of {c.length}
              </A>
              <_Component4 direction="row" spacing={0.25} sx={He}>
                <_Component5
...
const FloatingFeatureAnnouncements = o.memo(Qe);
export { FloatingFeatureAnnouncements };
