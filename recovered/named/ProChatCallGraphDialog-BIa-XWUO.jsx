...
const ke = (r, t) => (a) => {
  ...
  return {
    ...
    boxShadow: t
      ? `0 0 0 3px ${j(s, 0.16)}, 0 18px 36px ${j(s, 0.2)}`
      : `0 12px 24px ${j(a.palette.text.primary, 0.08)}`,
    cursor: "pointer",
  };
};
const De = (r) => (t) => {
...
const I = (r, t) => {
  ...
  for (const i of r.filter((c) => c.kind === "review_join")) {
    ...
    if (c.length !== 1) {
      throw new Error(
        `Pro chat review join '${i.id}' must have exactly one outgoing edge.`,
      );
    }
    const o = a.get(c[0].target);
    if ((o == null ? undefined : o.kind) !== "ready_gate") {
      throw new Error(
        `Pro chat review join '${i.id}' must connect directly to a ready gate.`,
      );
    }
    const w = t.filter((x) => x.target === o.id);
    if (w.length !== 1 || w[0].source !== i.id) {
      throw new Error(
        `Pro chat ready gate '${o.id}' must have exactly one incoming review join edge.`,
      );
    }
    if (o.roundIndex !== i.roundIndex) {
      throw new Error(
        `Pro chat review join '${i.id}' and ready gate '${o.id}' belong to different rounds.`,
      );
    }
    if (p.has(o.id)) {
      throw new Error(
        `Pro chat ready gate '${o.id}' is connected to multiple review joins.`,
      );
    }
    s.set(i.id, o.id);
    p.add(o.id);
...
const Re = ({ data: r, selected: t }) => {
  const { incomingCount: a, node: s, outgoingCount: p } = r;
  const h = z(s);
  if (y(s)) {
    return (
      <_Component sx={De(s.status)} title={`${s.label} · ${S(s.status)}`}>
        <N type="target" position={P.Left} />
        <N type="source" position={P.Right} />
      </_Component>
    );
  } else {
    return (
      <_Component sx={ke(s.status, t)}>
        <N type="target" position={P.Left} />
        <_Component sx={Ne}>
          <_Component2 variant="graphNodeTitle" noWrap={true}>
            {s.label}
          </_Component2>
          <O label={S(s.status)} color={Se(s.status)} size="xs" />
        </_Component>
        ...
        <_Component sx={_e}>
          <_Component2 variant="caption" color="textSecondary" noWrap={true}>
            {s.roundIndex === null ? "Run" : `Round ${s.roundIndex}`}
          </_Component2>
          {h && <O label={`Elapsed ${h}`} size="xs" />}
          <_Component2 variant="caption" color="textSecondary" noWrap={true}>
            in {a} | out {p}
          </_Component2>
        </_Component>
...
const _ = ({ label: r, value: t }) => (
  <_Component sx={fe}>
    ...
    <_Component2 variant="contextMetricValue" color="textPrimary" noWrap={true}>
      {t}
    </_Component2>
  </_Component>
);
const We = ({ attachmentId: r, title: t }) => {
  const a = $(r);
  if (a.objectUrl) {
    return (
      <_Component
      ...
      >
        ...
        >
          <Ie
            ariaLabel={`Current search ${t} preview`}
            loadErrorMessage={`Could not load the round ${t.toLowerCase()}.`}
            pdfUrl={a.objectUrl}
            renderErrorMessage={`Could not render the round ${t.toLowerCase()}.`}
            scale={1}
            sx={{
              height: "100%",
            }}
    ...
    return (
      ...
      >
        {a.isLoading && <M size={28} />}
        <_Component2
          variant="body2"
          color={a.error ? "error" : "textSecondary"}
          align="center"
        >
          {!r && "No current research PDF is available."}
          {r &&
            a.error &&
            `Could not load the current research ${t.toLowerCase()}.`}
          {r &&
            !a.error &&
            `Preparing the current research ${t.toLowerCase()} preview...`}
        </_Component2>
      </_Component>
    );
  }
};
const Fe = ({ roundIndex: r, roundOutput: t }) => {
  const a = t == null ? undefined : t.artifacts.answerTex.pdfAttachmentId;
  const s =
    t == null ? undefined : t.artifacts.researchNotesTex.pdfAttachmentId;
  ...
  if (r === null) {
    return (
      <_Component sx={b}>
        <_Component2 variant="subtitle2">Current research PDFs</_Component2>
  ...
  const x = async (R) => {
    ...
    const Y = v
      ? `math-vision-pro-round-${r}-answer.pdf`
      : `math-vision-pro-round-${r}-research-notes.pdf`;
  ...
  return (
    <_Component sx={b}>
      ...
      >
        ...
        >
          <H value="answer" disabled={!a} aria-label="Preview answer PDF">
            Answer PDF
          </H>
          <H
            value="researchNotes"
        ...
        >
          <T
            ...
            variant="outlined"
            disabled={!a || i !== null}
            onClick={() => void x("answer")}
            startIcon={<_Component4 />}
          >
            {i === "answer" ? "Downloading..." : "Download answer PDF"}
          </T>
          <T
            size="small"
            variant="outlined"
            disabled={!s || i !== null}
            onClick={() => void x("researchNotes")}
            startIcon={<Pe />}
          >
            {i === "researchNotes"
              ? "Downloading..."
      ...
      <We attachmentId={f ?? null} title={J} />
      {!a && (
...
const Me = ({
  ...
  if (!r) {
    return (
      <_Component sx={A}>
        <_Component2 variant="subtitle1">Highlighted workflow step</_Component2>
        <_Component2 variant="body2" color="textSecondary">
  ...
  const i = [...u.map((o) => o.source), ...l.map((o) => o.target)]
    .map((o) => m.get(o))
  ...
  return (
    <_Component sx={A}>
      <_Component sx={ue}>
        <_Component sx={ge}>
          <_Component2 variant="subtitle1" noWrap={true}>
            {r.label}
          </_Component2>
          <_Component2 variant="caption" color="textSecondary" noWrap={true}>
            {r.id}
      ...
      >
        <_ label="Kind" value={S(r.kind)} />
        <_ label="Status" value={S(r.status)} />
        <_ label="Round" value={r.roundIndex ?? "run"} />
        {h && <_ label="Runtime" value={h} />}
      </_Component>
      {E(r) && (
        <e.Fragment>
          <D />
          <_Component sx={b}>
            <_Component2 variant="subtitle2">Step detail</_Component2>
            <_Component2 variant="body2" color="textSecondary">
              {E(r)}
            </_Component2>
          </_Component>
      ...
      <D />
      <_Component sx={b}>
        <_Component2 variant="subtitle2">Streamed reasoning</_Component2>
        {r.streamReasoning.trim() && (
          <L content={r.streamReasoning} mode="compact" />
        )}
        {!r.streamReasoning.trim() && (
          <_Component2 variant="body2" color="textSecondary">
        ...
        {r.streamReasoningTruncated && (
          <_Component2 variant="caption" color="warning">
      ...
      {r.kind === "compute" && (
        <e.Fragment>
          <D />
          <_Component sx={b}>
            <_Component2 variant="subtitle2">Summarized output</_Component2>
            {r.displaySummary.trim() && (
              <L content={r.displaySummary} mode="compact" />
            )}
            {!r.displaySummary.trim() && (
              <_Component2 variant="body2" color="textSecondary">
      ...
      <Fe
        ...
        roundOutput={c}
        key={r.roundIndex ?? "run"}
      />
      <D />
      <_Component sx={b}>
        <_Component2 variant="subtitle2">Connected workflow steps</_Component2>
        {i.map((o) => (
          <T
            color="secondary"
            disabled={y(o)}
...
const ProChatCallGraphDialog = ({
  ...
  const l = g.useMemo(
    () => I(s.state.workflowNodes, s.state.workflowEdges),
    [s.state.workflowEdges, s.state.workflowNodes],
  );
  const m = g.useMemo(() => $e(l.nodes, l.edges), [l.edges, l.nodes]);
  const i = g.useMemo(() => Ee(l.nodes, l.edges, p), [p, l.edges, l.nodes]);
  const c = g.useMemo(
    () =>
      l.nodes.find((f) => f.id === h && !y(f)) ??
      l.nodes.find((f) => f.status === "running" && !y(f)) ??
      l.nodes.find((f) => !y(f)) ??
      null,
    [h, l.nodes],
  );
  ...
  const w = g.useMemo(
    () =>
      [
        s.id,
        ...m.map((x) => `${x.id}@${x.position.x},${x.position.y}`),
        ...i.map((x) => `${x.source}->${x.target}`),
      ].join("|"),
    [i, m, s.id],
  );
  return (
    <_Component1 open={a} onClose={t} fullScreen={true}>
      <_Component6 sx={se}>
        <_Component sx={ie}>
          <_Component2 variant="subtitle1" noWrap={true}>
          ...
          <_Component2 variant="caption" color="textSecondary" noWrap={true}>
            Math Vision Pro call message index {r} · {m.length} workflow steps ·{" "}
      ...
      >
        <_Component sx={ne}>
          ...
          {m.length > 0 && (
            ...
            >
              <_Component7 />
              <_Component8 showInteractive={false} />
            </_Component9>
          )}
        </_Component>
        <Me
          ...
          run={s}
          onSelectNode={u}
          workflowEdges={l.edges}
          workflowNodes={l.nodes}
        />
      </_Component0>
    </_Component1>
  );
};
export { ProChatCallGraphDialog };
