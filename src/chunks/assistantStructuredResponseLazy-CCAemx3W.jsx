...
import { D as Me } from "./Divider-BcuihGce.jsx";
import { E as $e, L as Ie } from "./LoadingSection-wWBu2CqJ.jsx";
import {
  c as _Component6,
  b as Oe,
  d as Le,
...
import { P as He } from "./TextField-D8vc_sXz.jsx";
import { i as Ke, a as _Component0 } from "./Markdown-Dh8LhaRc.jsx";
import { t as E } from "./MarkdownHighlights-C-ZhT8z1.jsx";
/**
...
const ie = {
  ...
  serverError: {
    ...
    description:
      "We couldn’t load a preview for this source. Please try again.",
    color: "error",
    retry: true,
  },
  limitExceeded: {
...
const Fe = (e) => {
  ...
  return (
    t ||
    (e.year === null || e.year === undefined ? null : `Published ${e.year}`)
  );
};
const Ze = (e) => {
  var t;
  return (
    ((t = e == null ? undefined : e.pdfLabel) == null ? undefined : t.trim()) ||
    ae
  );
};
const qe = (e) => {
...
const D = ({ label: e, onClose: t, status: r }) => {
  const o = c.useMemo(() => Ye(r), [r]);
  return (
    <n.Fragment>
      <_Component3 sx={o}>
        <U label={e} size="small" />
        <_Component2 aria-label="Close source preview" onClick={t}>
          <_Component />
        </_Component2>
      </_Component3>
      <Me />
...
const Q = ({
  ...
  const u = c.useMemo(
    () => (i === null ? "Try again" : `Try again ${i}s`),
    [i],
  );
  if (l.retry) {
    return (
      <_Component3 sx={V}>
        <_Component3 sx={et}>
          <X variant={l.color}>
            <_Component4 size={24} aria-hidden="true" />
          </X>
          <S variant="subtitle2" align="center" color={l.color}>
            {l.title}
          </S>
          <S variant="body2" align="center" color="textSecondary">
            {l.description}
          </S>
          <div>
            {l.retry && s ? (
    ...
    return (
      <_Component3 sx={V}>
        ...
        <S variant="subtitle2" color={l.color}>
          {l.title}
        </S>
        <S variant="body2" color="textSecondary">
          {l.description}
        </S>
        <B href={e} />
      </_Component3>
...
const dt = 5000;
const _Component7 = ({ href: e, isOpen: t, onClose: r }) => {
  ...
  if (i.isPending) {
    return (
      <n.Fragment>
        <D label={w} onClose={r} />
        <_Component3 sx={V}>
          <Ie title="" subtitle="Loading source metadata..." />
          <B href={e} />
        </_Component3>
      </n.Fragment>
    );
  ...
  if (A.status !== "success") {
    return (
      <n.Fragment>
        <D label={w} onClose={r} status={A.status} />
        <Q
          href={e}
          status={A.status}
          isRetryDisabled={b}
  ...
  if (!x) {
    throw new Error(
      "Citation metadata response succeeded without title or abstract.",
    );
  }
  return (
    <n.Fragment>
      <D label={$} onClose={r} status="success" />
      <_Component3 sx={Je}>
        <_Component3>
          <S variant="subtitle1" color="primary">
            {x.title}
          </S>
          {_ ? <S variant="caption">{_}</S> : null}
        </_Component3>
        <_Component3 sx={tt(true)}>
          <S variant="body1">{x.abstract}</S>
        </_Component3>
      </_Component3>
      <_Component3 sx={rt}>
        {P ? (
...
const _Component8 = ({
  ...
  const $ = c.useMemo(
    () => (h ? `${a}-assistant-citation-preview` : undefined),
    [a, h],
  );
  const N = c.useMemo(
    () => ({
  ...
  if (s) {
    return (
      <n.Fragment>
        <_Component3 component="span" ref={u} sx={xt}>
          <U
            className={t}
            id={o}
            title={l}
        ...
        <He
          ...
          slotProps={v}
        >
          <_Component3 data-source-url={r}>
            <_Component7 href={r} isOpen={h} onClose={j} />
          </_Component3>
        </He>
      </n.Fragment>
    );
...
const Tt = (e) => {
  var t;
  return (
    ((t = e == null ? undefined : e.match(ht)) == null ? undefined : t[1]) ??
    null
  );
};
const J = (e) => {
  ...
  for (const [s, i] of (e.toolCalls ?? []).entries()) {
    ...
    if (Array.isArray(a)) {
      for (const [u, p] of a.entries()) {
        ...
        const b = J(y) ?? J(T);
        const h =
          ((o = I(d.name)) == null ? undefined : o.trim()) ||
          `Generated artifact ${u + 1}`;
        ...
        .map((r) =>
          pe.test(r)
            ...
            : r.replace(ee, (o) => {
                var i;
                if (
                  (((i = o.match(wt)) == null ? undefined : i.length) ?? 0) > 1
                ) {
                  return `
...
const Nt = (e) => {
  ...
  const r = t.match(It);
  const o = (
    (r == null ? undefined : r[1]) ??
    (r == null ? undefined : r[2]) ??
    t
  ).trim();
  if (o) {
    return ` (${o})`;
...
const Pt = (e) => {
  ...
  let l = E(s, (a) => {
    let u = a;
    for (const { content: p, marker: d } of o) {
      u = u.replaceAll(d, () => p);
    }
    return u;
  });
  for (const { content: a, marker: u } of o) {
    l = l.replaceAll(u, () => `\`${a}\``);
  }
  return l;
};
const zt = (e) => {
  ...
  return E(t, (r) =>
    r
      .replace(
        /\b([A-Za-z](?:_[A-Za-z0-9{}]+)+)(?=\b|[^A-Za-z0-9])/g,
        (o, s) => `$${R(s)}$`,
      )
      .replace(
        /([α-ωΑ-Ω](?:_[A-Za-z0-9{}]+)?(?:\([^()\n]{1,80}\))?)/g,
...
const Dt = (e, { unwrapCodeWrappedMath: t }) => {
  const r = me(e);
...
const Or = (e) =>
  Array.isArray(e) &&
  e.every(
    (t) => K(t) && typeof t.symbol == "string" && typeof t.meaning == "string",
  );
const Lr = (e) =>
  Array.isArray(e) &&
  e.every(
...
const Nr = (e) => {
  var t;
  return (
    ((t = Oe.find((r) => r.value === e)) == null ? undefined : t.label) ?? Z(e)
  );
};
const Xt = (e, t) => {
...
const kr = (e) => {
  ...
  const i = (r == null ? undefined : r.sectionOrder) ?? [
    "intuition",
    "variable_defs",
  ];
  return [
...
const Jt =
  ...
  (t) => ({
    ...
    backgroundColor:
      e === "error"
        ? m(t.palette.error.main, 0.06)
        : m(t.palette.primary.main, 0.07),
...
const Pr = ({ reasoning: e, durationSeconds: t }) => {
  const r = c.useId();
  const o = c.useId();
  const s = ge(e);
  return (
    <Pe disableGutters={true} elevation={0} sx={cr}>
      <_Component9 expandIcon={<De />} aria-controls={r} id={o} sx={lr}>
        <_Component3
          sx={{
            display: "flex",
          }}
        ...
        <S variant="caption">{Yt(t)}</S>
      </_Component9>
      <Be id={r} aria-labelledby={o} sx={ur}>
        <_Component0 content={s} mode="compact" sx={fe} />
      </Be>
    </Pe>
  );
};
const zr = ({
  ...
  return (
    <_Component1 variant="inset" sx={Jt()} aria-live={l}>
      <_Component3 sx={er(true)}>
        <_Component3 sx={tr}>
          <_Component5 size={14} thickness={5} sx={rr} />
          <S variant="subtitle2" color="primary" noWrap={true} sx={nr}>
            {i}
          </S>
          {s && (
            <S variant="subtitle2" color="textSecondary" sx={or}>
              {o}
            </S>
          )}
        </_Component3>
        <_Component3 sx={sr}>
          <Le runId={e} startedAt={t} color="primary" variant="elapsedTime" />
        </_Component3>
      </_Component3>
      {y && (
        <_Component3 ref={u} sx={dr} onScroll={T}>
          <_Component0 content={y} mode="compact" sx={[fe, pr]} />
        </_Component3>
      )}
    </_Component1>
  );
