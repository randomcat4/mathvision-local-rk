...
import { W as _Component11, l as A } from "./route.lazy-B7bltzFg.tsx";
import {
  g as we,
  L as _Component,
  F as Ee,
...
import { b as Ae } from "./TextField-D8vc_sXz.jsx";
import { L as O } from "./ListItemIcon-CaLBil7Q.jsx";
import { F as _Component5 } from "./file-plus-_oOf3yl8.js";
import { U as _Component7 } from "./upload-Df6L5ERk.js";
import { D as _Component8 } from "./Divider-BcuihGce.jsx";
import { E as _Component9 } from "./LoadingSection-wWBu2CqJ.jsx";
import { P as _Component0 } from "./MarkdownHighlights-C-ZhT8z1.jsx";
...
import {
  ...
  o as vr,
  u as Cr,
} from "./useLatexProjectCreation-BfZAUJk-.js";
...
const Dr = n.memo(function ({
  ...
  const L = n.useMemo(() => {
    var c;
    return (
      ((c = s.find((o) => o.id === f)) == null ? undefined : c.path) ?? null
    );
  }, [s, f]);
  const R = n.useMemo(() => we(i, L), [i, L]);
  const y = n.useCallback(
    (c) => (a ? a.selectFile(c) : u(r.id, c.id)),
    [r.id, a, u],
  );
          ...
          Er(c.dataTransfer)
            ...
            .catch((o) => {
              I.error(
                o instanceof Error
                  ? o.message
                  : "Could not read the dropped project files.",
              );
            });
        }
      }
  ...
  return (
    ...
    >
      {s.length === 0 && (
        <F sx={re}>
          <G variant="caption" color="textSecondary" align="center">
      ...
      {s.length > 0 && (
        <_Component
          ...
          selectedFileId={f}
          onCreate={a == null ? undefined : a.createNode}
          onDelete={a == null ? undefined : a.deleteFile}
          onExpandedFolderPathsChange={b}
          onMove={a == null ? undefined : a.moveFile}
          onRename={a == null ? undefined : a.renameFile}
          onSelect={y}
          onSetMain={a == null ? undefined : a.setMainFile}
        />
      )}
      {(a == null ? undefined : a.isUnavailable) && (
        <F sx={re}>
          <G variant="caption" color="textSecondary" align="center">
            Project file contents could not be loaded.
          </G>
          <Z size="small" onClick={a.retryLoad}>
            Try again
          </Z>
        </F>
      )}
      {d && (
        <F sx={Lr}>
          <G variant="subtitle1" color="primary">
...
const Fr = ({
  ...
  return (
    <e.Fragment>
      <F className="latex-project-row-actions" sx={Rr}>
        {s.isPinned && <_Component2 label="Pinned LaTeX project" />}
        <_Component4
          id={o}
          className="latex-project-menu-button"
          data-sidebar-row-action-trigger="true"
          aria-label={`Project actions for ${s.name}`}
          aria-controls={c ? l : undefined}
      ...
      >
        <Oe kind="latex-project" title={s.name} />
        {w === "menu" && (
          <e.Fragment>
            <Pe disabled={i} isPinned={s.isPinned} onClick={() => p(L)} />
            <T dense={true} disabled={i} onClick={() => D("move")}>
              <O>
                <_e size={16} />
              </O>
              Move to folder
            ...
            <T dense={true} disabled={i} onClick={() => p(E)}>
              <O>
                <_Component0 size={16} />
              </O>
              Rename
            </T>
            <T dense={true} disabled={i} onClick={() => p(b)}>
              <O>
                <_Component1 size={16} />
              </O>
              Make a copy
            </T>
            <T dense={true} disabled={i} onClick={() => p(j)}>
              <O>
                <Ee size={16} />
              </O>
              Export project ZIP
            </T>
            <T dense={true} disabled={i} onClick={() => p(d)}>
              <O>
                <_Component10 size={16} />
              </O>
              Open in Overleaf
            </T>
            <T dense={true} disabled={i} sx={Xe} onClick={() => p(h)}>
              <O>
                <We size={16} />
              </O>
              Delete
...
const Br = n.memo(function ({
  ...
  const Q = (
    <e.Fragment>
      <F aria-hidden="true" sx={zr}>
        <_Component12 size={z.iconSize} />
      </F>
      <F sx={$r}>
        {P.isRenaming && (
          <Ke
        ...
        {!P.isRenaming && (
          ...
          >
            <_Component13 title={r.name}>
              <G variant={z.labelVariant} color="inherit" noWrap={true}>
                {r.name}
              </G>
            </_Component13>
          </He>
  ...
  return (
    <F data-active-explorer-path={f ? "true" : undefined} sx={Tr}>
      <F
        data-actions-open={$ ? "true" : undefined}
      ...
      >
        ...
        >
          <Ge isExpanded={j} size={15} />
        </_Component15>
        {P.isRenaming && <F sx={te(b, r.isPinned)}>{Q}</F>}
        {!P.isRenaming && (
          <_Component16
            ref={J}
            aria-label={`Open LaTeX project ${r.name}`}
            aria-current={b ? "page" : undefined}
            aria-disabled={u ? true : undefined}
            tabIndex={u ? -1 : undefined}
            to="/chats"
            search={me({}, r.id)}
            sx={te(b, r.isPinned)}
            onMouseEnter={() => {
      ...
      {r.itemCount > 0 && (
        <Ye gap="0.125rem" isExpanded={j} isInActivePath={f} />
      )}
      <Ze isExpanded={j} sx={Wr}>
        <F sx={Xr}>
          <Dr
            document={r}
            files={a}
...
const Ur = ({ documents: t, onSelectProject: r }) => {
  const { mutateAsync: s } = cr();
  const { mutateAsync: i } = dr();
  const { mutateAsync: a } = pr();
  const { mutateAsync: h } = xr();
  const { mutateAsync: j } = ur();
  const { mutateAsync: f } = mr();
  ...
  const w = n.useCallback(
    async (o, l) => {
      ...
      try {
        ...
        I.success(
          l ? "Project moved to folder." : "Project moved to Explorer.",
          {
            action: (C) =>
              n.createElement(
                Z,
  ...
  const D = n.useCallback(
    async (o) => {
      ...
      try {
        if (
          (await hr(o.id)).files.filter((S) => S.kind !== "folder").length > ee
        ) {
          throw new Error(
        ...
        if (l.closed) {
          throw new Error(
            "The Overleaf tab was closed before the project was ready.",
          );
        }
        await vr({
          archive: p,
        ...
        return {
          ...
          node: (
            <Br
              ...
              fileActions={
                W && (E == null ? undefined : E.documentId) === m.id ? E : null
              }
              files={q}
              isActionDisabled={a || j || x}
              isActionPending={(X == null ? undefined : X.documentId) === m.id}
  ...
  const P = n.useMemo(() => {
    ...
    if (f) {
      return (
        <F sx={ae}>
          <G variant="caption" color="textSecondary" align="center">
            LaTeX projects could not be loaded.
          </G>
          <Z
            size="small"
            disabled={b}
...
const Zr = ({
  ...
  const { value: o, updateValue: l } = wr({
    storageKey: Hr,
    defaultValue: qr,
    parse: Gr,
  ...
  const { createProject: $, isCreatingProject: N } = Cr();
  ...
  return (
    <e.Fragment>
      ...
      {x && (
        <n.Suspense fallback={null}>
          <Jr open={true} isCreating={N} onClose={K} onCreate={U} />
        </n.Suspense>
      )}
    </e.Fragment>
  );
};
const WorkspaceExplorer = (t) => (
  <Le>
    <Zr {...t} />
  </Le>
);
