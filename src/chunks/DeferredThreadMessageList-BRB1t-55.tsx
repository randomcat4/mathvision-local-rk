...
import { M as _Component7 } from "./Markdown-Dh8LhaRc.jsx";
import { M as Ue } from "./MChip-DiH8NXBU.jsx";
import { M as _Component } from "./maximize-2-B07z4fzS.js";
import { u as ut } from "./ChatInteractionOverrides-dyfOoEnd.js";
import { U as _Component2, A as _Component3 } from "./UserMessage-DN7iPijg.tsx";
import {
  h as mt,
  k as ht,
  l as xt,
...
import { L as Et } from "./ListItemIcon-CaLBil7Q.jsx";
import { L as Mt } from "./ListItemText-CBxudZz_.jsx";
import { C as _t } from "./NextStepsResponse-NcFjw3w7.jsx";
    ...
    <Ae variant="inset" sx={Lt}>
      <E sx={wt}>
        <E sx={Be}>
          ...
          />
          <P variant="body2" color="textPrimary" noWrap={true}>
            {$e(e)}
          </P>
        </E>
        {r && (
          <E data-message-index={t} sx={Be}>
            <Ue
            ...
            />
            <P variant="body2" color="textSecondary" noWrap={true}>
              {$e(r)}
            </P>
          </E>
        )}
  ...
  if (e.role === "user") {
    return (
      <_Component2
        ...
        onMinimize={K ? G : undefined}
        onOpenAdminModelExecution={Y ? ie : undefined}
        onQuoteClick={ge}
      />
    );
  } else {
    return (
      <_Component3
        ...
        onContentResizeInteraction={X}
        onBranch={h || q ? fe : undefined}
        onFeedbackChange={h ? Te : undefined}
        onBookmarkChange={
          h && $ && (!e.proChatCall || e.proChatCall.status === "completed")
            ? be
            : undefined
        }
        onSelectNextStep={h && !N ? me : undefined}
        onRunNextSteps={he}
        onRunAllNextSteps={xe}
        onOpenAdminModelExecution={Y ? ie : undefined}
      />
...
const ae = (e, r) => {
  ...
  return [
    J(e, r),
    _e(e.content),
    _e(((t = e.quote) == null ? undefined : t.text) ?? ""),
    _e(e.reasoning ?? ""),
...
const qt = (e, r, t) => {
  ...
  for (let o = 0; o < e.length; o += 1) {
    ...
    const h =
      r &&
      m.role === "user" &&
      (d == null ? undefined : d.role) === "assistant" &&
      d.isMinimized &&
      !I
        ? d
        : null;
    const S =
      m.role === "user" && (d == null ? undefined : d.role) !== "assistant";
...
const D = (
  e,
  { charsPerLine: r = $t, lineHeightPx: t = Dt, maxHeightPx: s = Ft } = {},
...
const Vt = (e) =>
  Array.isArray(e)
    ...
    : Me;
const Ce = (e) =>
  (e == null ? undefined : e.operation) === "writeup" &&
...
const Xt = (e) => {
  ...
  return (
    ((r = e.agentExecution) == null
      ? undefined
      : r.outputs.some((t) => t.operation === "writeup" && Ce(t.response))) ??
    false
  );
};
...
const ir = ({
  ...
  const x = a.useCallback(
    (i, l) => {
      const g =
        (l == null ? undefined : l.contentRect.height) ??
  ...
  const L = a.useCallback(
    (i, { align: l = "center", behavior: g = "smooth" } = {}) => {
      ...
      const k =
        (z = o.current) == null
          ? undefined
          : z.querySelector(`[data-thread-row-index="${i}"]`);
      if (k) {
        k.scrollIntoView({
          behavior: g,
          block: nr(l),
...
const dr = ({ branchSpaceId: e, lineage: r }) => {
  const t = a.useId();
  ...
  return (
    <E sx={sr}>
      <Ae variant="inset" sx={ar}>
        <E aria-hidden="true" sx={or}>
          <Ee size={17} strokeWidth={2.2} />
        </E>
        <E sx={cr}>
          <P variant="subtitle2">{M.title}</P>
          <P variant="caption" color="textSecondary" noWrap={true}>
            {M.detail}
          </P>
        </E>
        {r.parent && (
          <He
            ...
            color="secondary"
            variant="outlined"
            startIcon={<Ct size={15} />}
            tooltip={`Open branch source: ${r.parent.title}`}
            sx={Re}
          >
            <P variant="body2" noWrap={true}>
              From {r.parent.title}
            </P>
          </He>
        )}
        ...
        {r.children.length > 1 && (
          <Ke
            ...
            color="secondary"
            variant="outlined"
            endIcon={<_Component4 size={15} />}
            aria-controls={s ? t : undefined}
            aria-haspopup="menu"
            aria-expanded={s ? true : undefined}
            onClick={d}
            sx={Re}
          >
            {r.children.length} branches
          </Ke>
        )}
      </Ae>
      {r.children.length > 1 && (
        ...
        >
          {r.children.map((f) => (
            ...
            >
              ...
              <Mt primary={f.title} secondary="Starts from this answer" />
            </_Component5>
          ))}
        </It>
      )}
...
const Je = a.forwardRef(
  (
    ...
    const W = a.useMemo(() => {
      ...
      if (
        (u == null ? undefined : u.operation) !== "lit-search" ||
        !mt(u.bibliography)
      ) {
        return [];
      } else {
    ...
    const { messageIndexToRowIndex: Ie, rows: V } = a.useMemo(
      () => qt(s, d, f),
      [f, s, d],
    );
    const {
      ...
      if (c.kind === "minimizedStandaloneUser") {
        return (
          ...
          >
            <De
              ...
              queryNumber={(we = p.get(u)) == null ? undefined : we.queryNumber}
              isHighlighted={I === u}
              isRestoreDisabled={L}
              onRestore={() => {
                X(u, null, false);
        ...
        return (
          ...
          >
            <De
              ...
              queryNumber={(ze = p.get(u)) == null ? undefined : ze.queryNumber}
              responseNumber={
                (Ne = p.get(c.assistantMessageIndex)) == null
                  ? undefined
                  : Ne.responseNumber
              }
        ...
        return (
          <E
            ...
            sx={ke}
          >
            <_Component7 macros={M[u]}>
              <Pt
                chatId={e}
                message={c.message}
                messageIndex={u}
                adminHasRetryActivity={Q != null && !!Q.get(u)}
                adminModelExecutionRunId={G == null ? undefined : G.get(u)}
                queryNumber={
                  (Oe = p.get(u)) == null ? undefined : Oe.queryNumber
                }
                responseNumber={
                ...
                onSetAssistantMessageBookmark={xe}
                onSelectNextStep={ne}
                onRunNextSteps={u === i ? ge : undefined}
                onRunAllNextSteps={u === l ? F : undefined}
                onSetMessagePairMinimized={X}
                onJumpToMessageIndex={q}
                onOpenAdminModelExecution={fe}
                onContentResizeInteraction={Te}
    ...
    if (tt) {
      ...
      return (
        <E ref={ve} sx={fr}>
          {V.map((c, u) => (
            <E data-thread-row-index={u} key={c.key}>
              {Le(c)}
            </E>
          ))}
        </E>
...
const _Component1 = ({ selection: e, onQuote: r, onClose: t }) => (
  <Ze
    open={e !== null}
    anchorEl={(e == null ? undefined : e.anchorEl) ?? null}
    placement="top"
    modifiers={br}
    sx={Ir}
  >
...
const _Component10 = ({
  ...
  const f = a.useMemo(() => {
    ...
    for (const [y, U] of e.entries()) {
      ...
      const T = e[y + 1];
      const j =
        (T == null ? undefined : T.role) === "assistant" ? le(T.content) : null;
      const L =
        (T == null ? undefined : T.role) === "assistant" &&
        typeof (j == null ? undefined : j.operation) == "string"
          ? j.operation
          : (T == null ? undefined : T.role) === "assistant" &&
  ...
  if (p) {
    ...
    return (
      ...
      >
        <Ge
          tooltip={_ && !I ? "User queries" : undefined}
          sx={[Rr, ...(Array.isArray(t) ? t : t ? [t] : [])]}
          aria-label="User queries"
          aria-controls={h}
          aria-haspopup={_ ? "dialog" : undefined}
        ...
        >
          <E component="span" aria-hidden="true" sx={vr}>
            {Lr.map((x, A) => (
              <E
                component="span"
                sx={[
                ...
                >
                  {f.length > 0 ? (
                    f.map((x) => (
                      ...
                      >
                        {x.operation && (
                          <E component="span" aria-hidden="true" sx={Cr}>
                            <Tt operation={x.operation} size={20} />
                          </E>
                        )}
                        <_Component9 sx={Xe}>
                          <P variant="caption" color="primary" noWrap={true}>
                            Prompt {x.queryNumber}
                          </P>
                          <P variant="body2" color="textPrimary" noWrap={true}>
                            {x.preview}
                          </P>
...
const Nn = ({
  ...
  <n.Fragment>
    <_Component1 selection={p} onQuote={t} onClose={r} />
    <_Component10 messages={e} onSelectUserQuery={s} />
  </n.Fragment>
);
